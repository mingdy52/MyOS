...
async function main() {
  ...
  "projectNotes": {
    "0": { "description": "어떤 문제를 푸는 서비스인지 한 문장 + 본인이 맡은 역할 한 문장 (총 1~2문장)" }
  }
}`;
  ...
  const system =
    ...
    "3) projectNotes — 각 프로젝트의 'id' 값을 키로(문자열 그대로), 카드에 들어갈 요약을 쓴다. 모든 프로젝트에 대해 빠짐없이 작성한다.\n" +
    "   - 스타일: '어떤 문제를 푸는 서비스/시스템인지' 한 문장 + '본인이 맡은 역할·모듈' 한 문장. 총 1~2문장, 담백하고 친절하게.\n" +
    "   - 금지: 기술 스택 나열(예: 'Java·Spring으로'), '풀스택으로 개발한 실무 프로젝트입니다' 같은 틀에 박힌 상투구. 스택은 카드에 따로 표시되므로 문장에 넣지 않는다.\n" +
    "   - 메모가 빈약하면 빈약한 대로 사실만. 없는 팀 규모·역할·성과는 지어내지 않는다.\n" +
